import type { Question, QuestionBank, AnswerOption } from './schema';

export function normalizeAnswer(question: Question, value: string): number {
  // Simple normalization: find the option and get its points
  const selectedOption = question.options.find(opt => opt.text === value);
  if (!selectedOption || !selectedOption.points) return 0;
  
  // Normalize to 0-1 scale - assume max points is 10
  return Math.min(selectedOption.points / 10, 1);
}
...
export function calculateQuestionPoints(question: Question, value: string): number {
  ...
  // Return actual points earned (may be 0)
